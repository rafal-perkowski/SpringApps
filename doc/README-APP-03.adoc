...
** pliki JSP dla Spring Web Flow (.xml) nie zostały konwertowane na Thymeleaf (.html) ponieważ dla innych rozszerzeń niż (.jsp) i (.jspx) _FlowViewResolver_ zgłasza wyjątek _IllegalArgumentException_ w funkcji _getViewInternal()_ link:{flowviewresolver}[FlowResourceFlowViewResolver.java]
